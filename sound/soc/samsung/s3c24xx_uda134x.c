...
#ifdef CONFIG_CPU_S3C2416 //modified by Paul
static int LIMIT_RFS=3;
static int LIMIT_PRESCAL_RANGE=64;
static unsigned int rates[64*3];
#else
static int LIMIT_RFS=2;
static int LIMIT_PRESCAL_RANGE=33;
					...
					struct snd_pcm_hw_params *params)
{
	...
	long err, cerr;
	unsigned int div;
	int i, bi,bfs=S3C2410_IISMOD_32FS;


	err = 999999;
	bi = 0;
	for (i = 0; i < LIMIT_RFS*LIMIT_PRESCAL_RANGE; i++) {
...
#ifdef CONFIG_TQ2416_DEBUG_UDA1341_MACHINE
	printk("%s desired rate %lu, %d\n", __func__, rate, bi);
	printk("%s will use: 0x%x %s %d sysclk %d err %ld\n", __func__,
		 fs_mode, clk_source == S3C24XX_CLKSRC_MPLL ? "MPLLin" : "PCLK",
		 div, clk, err);
#endif
...
#endif
	ret = snd_soc_dai_set_clkdiv(cpu_dai, S3C24XX_DIV_BCLK,	bfs);
	if (ret < 0)
		return ret;

#ifdef CONFIG_CPU_S3C2416
	ret = snd_soc_dai_set_clkdiv(cpu_dai, S3C24XX_DIV_PRESCALER,S3C24XX_PRESCALE(div));

#else
	ret = snd_soc_dai_set_clkdiv(cpu_dai, S3C24XX_DIV_PRESCALER,S3C24XX_PRESCALE(div, div));
#endif
	if (ret < 0)
		return ret;
...
static int s3c24xx_uda134x_probe(struct platform_device *pdev)
{
	...
	//For TQ2416 BUTTONS,please don't remove them!
	s3c_gpio_cfgpin(S3C2410_GPG(0),S3C2410_GPG0_EINT8);
	s3c_gpio_cfgpin(S3C2410_GPG(1),S3C2410_GPG1_EINT9);
	s3c_gpio_cfgpin(S3C2410_GPG(2),S3C2410_GPG2_EINT10);
	return ret;
}
