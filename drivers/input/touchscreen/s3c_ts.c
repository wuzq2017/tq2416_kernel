...
static void touch_timer_fire(unsigned long data)
{
	...
	if (down) {
//		if (ts.count != 0 /*(1 << ts.shift)*/) { //lhh modify == -> >=		//HJ_add 20130221
		if (ts.count == (1 << ts.shift)) {		//HJ_add 20130221
			ts.xp >>= ts.shift;
			ts.yp >>= ts.shift;
...
static irqreturn_t stylus_irq(int irq, void *dev_id)
{
	...
	if (down){
		s3c_adc_start(ts.client, 0, 1 << ts.shift);
//		writel(readl(ts.io + S3C2410_ADCTSC) & (~(1<<8)), ts.io + S3C2410_ADCTSC);		//HJ_add 20130221
	}
	else
	{
//		writel(readl(ts.io + S3C2410_ADCTSC) | (1<<8), ts.io + S3C2410_ADCTSC);		//HJ_add 20130221
...
#endif
	ts.input->name = "TQ2416 TouchScreen";
...
static struct platform_driver s3c_ts_driver = {
	.driver         = {
#ifdef CONFIG_MACH_TQ2416
		.name   = "s3c2416-ts",//"samsung-ts",//
#else
		.name   = "s3c2410-ts",//"samsung-ts",//
#endif
		.owner  = THIS_MODULE,
#ifdef CONFIG_PM
		.pm	= &s3c_ts_pmops,
