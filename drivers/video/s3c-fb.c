...
extern struct s3c_fb_pd_win * get_s3c2416_fb(int num);
				      ...
				      struct s3c_fb_win **res)
{
	...
	*res = win;
	...
	dev_info(sfb->dev, "LCD type is %s, default_bpp=%d, pixclock=%d\n", windata->win_mode.name, windata->default_bpp, windata->win_mode.pixclock);
...
static int __devinit s3c_fb_probe(struct platform_device *pdev)
{
	...
	for (win = 0; win < fbdrv->variant.nr_windows; win++) 
	{
		printk("LCD type is %s, default_bpp=%d, pixclock=%d\n", pd->win[win]->win_mode.name, pd->win[win]->default_bpp, pd->win[win]->win_mode.pixclock);
	}
...
/* S3C2416 style hardware */
static struct s3c_fb_driverdata s3c_fb_data_s3c2416 = {
	.variant = {
		.nr_windows	= 2,
		.is_2443	= 1,
...
static struct platform_device_id s3c_fb_driver_ids[] = {
	{
		...
		.name		= "s3c2416-fb",
		.driver_data	= (unsigned long)&s3c_fb_data_s3c2416,
	},
	{},
};
